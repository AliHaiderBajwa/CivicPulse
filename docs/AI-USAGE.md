...
| M8 close-out | PR #21 approved by ashar1x, merged as 4e1d80d; A5 ticked; no evidence altered post-merge | The agent did not merge or approve on the partner's behalf — the review that unblocked this step is Ashar's own |
| M9 skeleton + data | Environment stood up from cold (uv 3.12 venv, cp-pg/cp-redis, PDF re-checked); PR #22 = issue #1 (nine endpoints, contract-named schemas, idempotent OpenAPI exporter, JSON logs, request middleware); PR #23 = issue #3 (Alembic 0001, repository, uuid5+ON CONFLICT idempotent seed, up/down/up + double-seed proof); Sonar S8707 cleared by removing the exporter's argv path entirely | The exporter now derives its one destination from `__file__`, so no caller-supplied path exists to traverse; the seed reports a before/after count instead of trusting psycopg3's `-1` rowcount; the docs conflict with Ali's direct dev commit was resolved by keeping *both* partners' PROGRESS entries and regenerating the docx, not by picking a side |
| M10 close-out | PR #22 merged after the Sonar fix; PR #23 re-synced through two dev moves and Ali's `simulated` TriagedBy requirement (3 commits: schema, re-export, `gen:api`); M9/M10 artifacts regenerated | The `TriagedBy` fix was a contract-vs-migration mismatch Ali caught in review — the agent applied his required change verbatim rather than re-litigating it; both dev collisions were resolved by keeping both sides |
| M11 routes + evidence | `feat/complaints-api`: services (complaint/stats/rate-limiter/health/meta), `Annotated` DI wiring, five route modules; evidence/25 built from a live HTTP run (201/400/404/409/429, cache MISS→HIT→invalidation, dead-DB `/health` 200 vs `/ready` 503, `rules:fallback` + WARNING on a failing provider); 12 rubric boxes ticked (C1–C4, E1–E3, F1–F3, F5, F6) | Every claim in evidence/25 is a raw transcript from the local stack, not a hand-written assertion; F4 was left unticked because its hit-rate script does not exist yet; curl bodies were sent as files because PowerShell 5.1 mangles quoted JSON — the test harness issue, not the API |
| M12 image + drain | `backend/Dockerfile` + `.dockerignore` (Track Step 8, exact patch pin), cold build to 74.1 MB, context sizes reported for both images, live SIGTERM drain capture, AOF pointer; boxes C6/E4/G1/G2 ticked | The first context-size measurement used BuildKit's incremental transfer and was *wrong* (2 B — it resends only diffs); it was replaced with a fresh-builder build plus direct file enumeration, and the quirk is disclosed in evidence/26 rather than quoted as if it were a cold number |
| M13 test suite | `backend/pyproject.toml` (pytest/ruff/mypy config — the file that activates CI's lint-and-type, test-backend and integration jobs), `backend/tests/` 76 tests across 14 files, `entrypoint.sh` migrate-on-boot + Dockerfile ENTRYPOINT wiring, `.gitattributes` (LF for `*.sh`), ruff-driven line-length fixes across 14 app files, evidence/27 | The conftest guard admits CI's suffix-less disposable database only under `GITHUB_ACTIONS` instead of being loosened for everyone; the fakeredis-optional fallback exists because CI installs only pytest/pytest-cov/httpx and was verified in a throwaway venv running CI's exact install sequence; C7 was ticked only after five consecutive clean runs |
